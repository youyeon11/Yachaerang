<template>
  <div :class="['flex gap-3 max-w-[80%]', isAssistant ? 'self-start' : 'self-end']">
    <div
      v-if="isAssistant"
      class="flex items-center justify-center w-[35px] h-[35px] rounded-lg text-white font-bold text-sm shrink-0 bg-[#10a37f]"
    >
      <img
        v-if="aiProfile.avatarUrl"
    ...
    <div class="flex flex-col">
      ...
      <div
        class="px-[18px] py-[14px] rounded-xl text-[15px] leading-[1.6]"
        :class="isAssistant ? 'bg-[#f1f3f5] text-[#212121]' : 'bg-[#fecc21] text-[#212121]'"
      >
    ...
    <div
      v-if="!isAssistant"
      class="flex items-center justify-center w-[35px] h-[35px] rounded-lg text-white font-bold text-sm shrink-0 bg-[#5436da]"
    >
      <img
