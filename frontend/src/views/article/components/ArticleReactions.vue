      ...
      <div v-for="(emoji, type) in reactionIcons" :key="type" class="relative group w-full">
        ...
        >
          ...
          <div v-if="getReactorsByType(type).length > 0" class="w-full">
            <div v-for="user in getReactorsByType(type)" :key="user.nickname" class="flex items-center justify-center gap-2 mb-1">
          ...
          <div class="absolute top-full left-1/2 -translate-x-1/2 w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent border-t-[8px] border-t-gray-900"></div>
        </div>
        ...
        <button
          ...
          :class="[myReaction === type ? 'border-[#FECC21] shadow-md scale-105' : 'border-transparent shadow-sm hover:border-[#FECC21]/30 hover:shadow-xl hover:-translate-y-1']"
        >
          <span class="text-3xl mb-2 transition-transform duration-300 group-hover:scale-110" :class="{ 'scale-125': myReaction === type }">
            {{ emoji }}
          </span>
          <span class="text-[10px] md:text-xs font-bold text-gray-500 mb-1">{{ reactionLabels[type] }}</span>
          <span class="text-sm font-black text-gray-900">{{ reactions[type]?.count || 0 }}</span>
        </button>
      </div>
    </div>
  </div>
