        ...
        <tbody class="divide-y divide-slate-50 font-mono text-slate-700">
          ...
          >
            <td class="px-4 py-4 font-bold text-slate-500 font-sans flex items-center gap-1.5">
              ...
              <span
                v-if="row.isMax"
                class="text-xs bg-rose-500 text-white px-2 py-0.5 rounded font-black shrink-0"
                >최고</span
              >
              <span
                v-if="row.isMin"
                class="text-xs bg-blue-500 text-white px-2 py-0.5 rounded font-black shrink-0"
                >최저</span
              >
            </td>

            <td class="px-4 py-4 text-right font-black text-slate-900 text-lg tracking-tighter">
              <span v-if="row.thisPrice !== null && row.thisPrice !== undefined">{{ row.thisPrice.toLocaleString() }}원</span>
              <span v-else class="text-slate-300">-</span>
            </td>

            <td
              class="px-4 py-4 text-right font-bold text-base"
              :class="row.prevDiff !== null && row.prevDiff > 0 ? 'text-rose-500' : row.prevDiff !== null && row.prevDiff < 0 ? 'text-blue-500' : 'text-slate-400'"
            >
            ...
            <td
              class="px-4 py-4 text-right font-bold text-base"
              :class="row.prevRate !== null && row.prevRate > 0 ? 'text-rose-500' : row.prevRate !== null && row.prevRate < 0 ? 'text-blue-500' : 'text-slate-400'"
            >
            ...
            <td class="px-4 py-4 text-right font-bold text-slate-700 text-base tracking-tighter border-l border-slate-50">
              <span v-if="row.lastPrice !== null && row.lastPrice !== undefined"> {{ row.lastPrice.toLocaleString() }}원 </span>
              <span v-else class="text-slate-300">-</span>
            </td>

            <td
              class="px-4 py-4 text-right font-bold text-base"
              :class="row.yoyDiff !== null && row.yoyDiff > 0 ? 'text-rose-600' : row.yoyDiff !== null && row.yoyDiff < 0 ? 'text-blue-600' : 'text-slate-400'"
            >
            ...
            <td
              class="px-4 py-4 text-right font-bold text-base"
              :class="row.yoyRate !== null && row.yoyRate > 0 ? 'text-rose-600' : row.yoyRate !== null && row.yoyRate < 0 ? 'text-blue-600' : 'text-slate-400'"
            >
