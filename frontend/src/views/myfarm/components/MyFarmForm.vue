<template>
  <main class="flex-1 overflow-y-auto p-8 bg-gray-50">
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div class="flex flex-col sm:flex-row gap-3 pt-2">
            <button type="submit" class="flex-1 rounded-lg bg-[#F44323] px-4 py-2 font-medium text-white transition-colors hover:bg-[#d63a1f]">
              {{ isEdit ? "수정 완료" : "등록하기" }}
            </button>
            <button type="button" @click="showCancelModal = true" class="flex-1 rounded-lg border border-gray-300 bg-white px-4 py-2 font-medium text-gray-700 transition-colors hover:bg-gray-50">
              취소
            </button>
          </div>
        </form>
      ...
      <!-- 취소 확인 모달 -->
      <ConfirmModal :show="showCancelModal" title="정말 취소하시겠습니까?" message="취소하시면 작성 내용이 날아갑니다." @confirm="handleCancelConfirm" @cancel="showCancelModal = false" />

      <!-- 작물 선택 모달 -->
      <div v-if="isCropModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
        <div class="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in">
          <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            ...
            <button @click="isCropModalOpen = false" class="text-slate-300 hover:text-red-500 text-2xl font-light">×</button>
          </div>
          ...
          <div class="p-5 grid grid-cols-3 md:grid-cols-4 gap-2 max-h-[50vh] overflow-y-auto">
            <button
              ...
              :class="isSelected(item.label) ? 'border-red-500 bg-red-50 text-red-600 shadow-sm ring-1 ring-red-100' : 'border-gray-100 bg-gray-50/60 text-gray-600 hover:bg-gray-100'"
          ...
          <div class="p-4 bg-gray-50 border-t flex justify-end">
            <button @click="isCropModalOpen = false" class="bg-red-500 text-white px-6 py-2 rounded-lg font-bold hover:bg-red-600 transition-colors">선택 완료</button>
          </div>
        </div>
      </div>
    </div>
